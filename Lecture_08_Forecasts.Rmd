...
In this lecture we will revisit the model we built in the [Decision Models](#decision_models) lecture and the [Model programming](#model_programming) seminar to learn more about how to generate useful forecasts for communicating model results. We will implement the Monte Carlo simulation again. 

In the [Model programming](#model_programming) seminar we generated a model. As a quick referesher change the plot from `boxplot_density` to `smooth_simple_overlay` by using the `method` argument in the `plot_distributions` function. 
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/MlIetFhu87U" frameborder="0" allowfullscreen></iframe>

```{r schiffers-forecasts-question-1, echo=FALSE}
question("Why might it be useful to test for value of information?",
  answer("To determine what to do now given current evidence.", correct = TRUE),
  answer("To find out if we need more evidence.", correct = TRUE),
  answer("To determine if we should reject the null hypothesis."),
  answer("To find out what type of evidence we need and how much.", correct = TRUE),
  answer("To determine if we should wait to get the evidence.", correct = TRUE),
  incorrect = "Watch [the talk](https://youtu.be/MlIetFhu87U) and try again.",
allow_retry = TRUE
)
```
...
question("Why do we calculate Expected Value of Perfect Information (EVPI)?",
  ...
  answer("To determine how much money we can make by doing research."),
  incorrect = "Watch [the talk](https://youtu.be/MlIetFhu87U) and try again.",
allow_retry = TRUE
)
```
...
question("How is EVPI calculated?",
  ...
  answer("It is the expected value given perfect information (before decision making)
minus the expected maximum value.", correct = TRUE),
...
Projection to Latent Structures (PLS), also sometimes known as Partial Least Squares regression is a modeling technique that can deal with multiple collinear dependent and independent variables. It can be used as another means to assess the outcomes of a Monte Carlo model. Variable Importance in Projection (VIP) scores estimate the importance of each variable in the projection used in a PLS mode. A variable with a VIP Score close to or greater than 1 (one) can be considered important in given model. The input is a PLS model and the output is a set of column vectors equal in length to the number of variables included in the model. Read more in ['A Simple Explanation of Partial Least Squares' by Kee Siong Ng](http://users.cecs.anu.edu.au/~kee/pls.pdf).

In the seminar we will go into detail abut how to calculate these with the built-in functions of the `decisionSupport` package.
