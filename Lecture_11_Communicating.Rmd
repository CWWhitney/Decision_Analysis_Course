## Lecture 11: Communicating Decision Support {#communicating}
...
In the [Model programming](#model_programming) seminar we generated a model called `example_mc_simulation`. As a quick refresher change the plot of the results of that model from `hist_simple_overlay` to `boxplot_density` by using the `method` argument in the `plot_distributions` function. 
